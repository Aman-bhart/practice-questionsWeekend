...
const filterAdults = function (people) {
  return people.filter(function (data) {
    return data.age > 30;
  });
};
...
const filterLongBooks = function (books) {
  return books.filter(function (bookInfo) {
    return bookInfo.pages > 200;
  });
};
...
const filterHighGrades = function (students) {
  return students.filter(function (studentInfo) {
    return studentInfo.grade > 80;
  });
};
...
const filterInStockProducts = function (products) {
  return products.filter(function (productStatus) {
...
const filterRecentActiveUsers = function (users) {
  return users.filter(({ lastPostDate }) => isWithinDays(lastPostDate, 7));
};

// console.log(filterRecentActiveUsers([{username: "alice", lastPostDate: "2024-12-01", active: true}, {username: "bob", lastPostDate: "2024-11-20", active: true}]));
...
const filterStudentsWithAllSubjectsPassed = function (students) {
  return students.filter((studentData) =>
    studentData.subjects.every((subject) => subject.passed)
  );
};
...
const extractMonth = (date) => +date.split("-").at(1);

const filterBirthdaysThisMonth = function (people) {
  return people.filter(({ birthDate }) => extractMonth(birthDate) === 12);
};
...
const filterPopularPosts = function (posts) {
  const avg = calcAverage(posts.map(({ likes }) => likes));
...
const filterByDate = function (events, date) {
  const dateDays = getNumOfDays(date);

  return events.filter(({ date }) => getNumOfDays(date) - dateDays > 0);
};

// console.log(filterByDate([{name: "Event1", date: "2024-12-01"}, {name: "Event2", date: "2024-11-15"}],"2024-11-30"));
...
const filterBySalary = function (employees, salary) {
  const preferSalary = salary;

  return employees.filter(({ salary }) => salary > preferSalary);
};
